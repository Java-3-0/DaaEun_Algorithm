...
// BFS
public class Solution_5656_벽돌깨기_태희쌤 {
	...
	public static boolean go(int count, int[][] map) {	// 중복순열 이용하여 구슬 던지기, 벽돌이 다 부서졌다면 true, 아니면 false 리턴
	...
	public static void boom(int[][] map, int r, int c) {	// r, c 위치에서 주변의 가능한 모든 벽돌도 함께 부수는 처리
		Queue<Point> queue = new LinkedList<Point>();
		if(map[r][c] > 1) { // 벽돌크기가 2 이상이면
			queue.offer(new Point(r, c, map[r][c]));
		}
		map[r][c] = 0;	// 자신으 제거처리(빈 공간으로...) : visit 처리 역할
			...
			for (int d = 0; d < 4; d++) {
				...
				for (int k = 1; k < p.cnt; k++) {	// 벽돌의 크기 -1만큼 반복
					...
					if(nr>=0 && nr<H && nc>=0 && nc<W && map[nr][nc]>0) {
						...
						map[nr][nc] = 0;	// 빈 공간이면 그냥 0, 벽돌이면 제거처리 
					}
				}
			}
		}
	...
	public static void down(int[][] map) {	// 부서진 벽돌 정리, 남은 벽돌 정리!! (공중에 떠있는 벽돌도 아래로 내리기)
		for (int c = 0; c < W; c++) {
			int r = H-1;	// 아래행 시작
			while(r>0) {
